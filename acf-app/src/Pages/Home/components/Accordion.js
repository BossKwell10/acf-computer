...
export const Accordion = ({ faq }) => {
  ...
  return (
    <div>
      ...
      {show && (
        <div id={`accordion-flush-body-${faq.id}`} aria-labelledby={`accordion-flush-heading-${faq.id}`}>
          <div className="py-5 border-b border-gray-200 dark:border-gray-700">
            {reponse.split(". ").map((sentence, index) => (
              <p key={index} className="text-lg mb-2 text-gray-500 dark:text-gray-400">
                {sentence}.
              </p>
            ))}
          </div>
